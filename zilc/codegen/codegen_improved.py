...
class ImprovedCodeGenerator:
    ...
    def _get_operand_type_and_value(self, node: ASTNode) -> Tuple[int, int]:
        ...
        if isinstance(node, NumberNode):
            ...
            # The empty form <> is FALSE = constant 0. Returning (1, 0)
            # treated it as THE STACK: <EQUAL? ,PRSI <> ,ROOMS> compiled to
            # 'je PRSI,sp,ROOMS', popping a garbage word.
            if (isinstance(node.operator, AtomNode) and node.operator.value == '<>'
            ...
            if node.value in self._string_operand_to_placeholder:
                ...
                placeholder_idx = self._next_string_operand_index
                if placeholder_idx > self._max_string_operand_index:
                    # 0xFC00-band overflow (>256 unique code string operands):
                    # allocate from the wide data-string band instead. The
                    # positions of these markers in ROUTINE code are recorded
                    # structurally by _scan_placeholder_words and patched
                    # point-wise by the assembler (never by a byte scan).
                    self._string_code_overflow = True
                    return (0, self.register_data_string(node.value))
    ...
    def gen_clear(self, operands: List) -> bytes:
        if self.version < 4:
            if not getattr(getattr(self, "compiler", None), "_is_classic_parser", False):
                # ZILF-dialect / toy programs keep the hard V3 gate
                raise ValueError("CLEAR requires V4 or later")
            self._warn("CLEAR (erase_window) is V4+; emitting anyway for "
                       "extended-V3 interpreters (screen no-op in zwalker)")
        if len(operands) != 1:
            raise ValueError("CLEAR requires exactly 1 operand")
        code = bytearray()
        op_type, op_val = self._get_operand_type_and_value(operands[0])
        code.append(0xED)  # ERASE_WINDOW (VAR:0x0D)
        if op_type == 0 and (op_val < 0 or op_val > 255):
            # signed window (-1 unsplit+clear, -2 clear) must be a large constant
            code.append(0x3F)
            code.append((op_val >> 8) & 0xFF)
            code.append(op_val & 0xFF)
        else:
    ...
    def gen_erase(self, operands: List) -> bytes:
        if self.version < 4:
            if not getattr(getattr(self, "compiler", None), "_is_classic_parser", False):
                # ZILF-dialect / toy programs keep the hard V3 gate
                raise ValueError("ERASE requires V4 or later")
            self._warn("ERASE (erase_line) is V4+; emitting anyway for "
                       "extended-V3 interpreters")
        if len(operands) != 1:
            raise ValueError("ERASE requires exactly 1 operand")
        code = bytearray()
        op_type, op_val = self._get_operand_type_and_value(operands[0])
        code.append(0xEE)  # ERASE_LINE (VAR:0x0E)
        type_byte = 0x01 if op_type == 0 else 0x02
        code.append((type_byte << 6) | 0x3F)
        code.append(op_val & 0xFF)
        return bytes(code)
    ...
    def gen_curset(self, operands: List) -> bytes:
        if self.version < 4:
            if not getattr(getattr(self, "compiler", None), "_is_classic_parser", False):
                # ZILF-dialect / toy programs keep the hard V3 gate
                raise ValueError("CURSET requires V4 or later")
            self._warn("CURSET (set_cursor) is V4+; emitting anyway for "
                       "extended-V3 interpreters")
        code = bytearray()
        code.append(0xEF)  # SET_CURSOR (VAR:0x0F)
        if len(operands) == 1:
            if self.version < 6:
    ...
    def gen_get_cursor(self, operands: List) -> bytes:
        if self.version < 4:
            if not getattr(getattr(self, "compiler", None), "_is_classic_parser", False):
                # ZILF-dialect / toy programs keep the hard V3 gate
                raise ValueError("CURGET requires V4 or later")
            self._warn("CURGET (get_cursor) is V4+; emitting anyway for "
                       "extended-V3 interpreters")
        if len(operands) != 1:
    ...
    def gen_bufout(self, operands: List) -> bytes:
        if self.version < 4:
            if not getattr(getattr(self, "compiler", None), "_is_classic_parser", False):
                # ZILF-dialect / toy programs keep the hard V3 gate
                raise ValueError("BUFOUT requires V4 or later")
            self._warn("BUFOUT (buffer_mode) is V4+; emitting anyway for "
                       "extended-V3 interpreters")
        if len(operands) != 1:
            raise ValueError("BUFOUT requires exactly 1 operand")
        code = bytearray()
        op_type, op_val = self._get_operand_type_and_value(operands[0])
        # BUG FIX: buffer_mode is VAR:0x12 = 0xF2 (0xF1 is set_text_style --
        # BUFOUT was emitting the WRONG opcode)
        code.append(0xF2)
        if op_type == 1:
            code.append(0xBF)
        else:
    ...
    def gen_dless(self, operands: List[ASTNode]) -> bytes:
        ...
        # Need to skip: PUSH #0 (3 bytes) + JUMP ?end (3 bytes) = 6 bytes
        # Offset = 6 + 2 = 8
        code.append(0xC8)  # branch true, short, offset 8
        ...
        # JUMP past true case (skip 3 bytes: offset 5)
        code.append(0x8C)
        code.append(0x00)
        code.append(0x05)
    ...
    def gen_nequal(self, operands: List[ASTNode]) -> bytes:
        ...
        """
        if len(operands) < 2:
            return bytes([0xE8, 0x7F, 0x00])  # PUSH #0 for malformed call
        # <> comparand is the constant FALSE (0); see gen_equal.
        ...
        if needs_large1 or needs_large2:
            ...
            false_case_size = 3 + 3  # PUSH + JUMP
            true_case_start = false_case_size + 2  # Z branch: offset = skip + 2
            ...
            # Branch on FALSE (not equal) to true case (offset 9 bytes forward)
            # Branch byte: bit 7 = 0 (branch on false), bit 6 = 1 (short form)
            code.append(0x48)  # branch on false, short, offset 8

        # FALSE case (values ARE equal): PUSH #0
        code.append(0xE8)
        code.append(0x7F)
        code.append(0x00)
        ...
        # TRUE case (values are NOT equal): PUSH #1
        code.append(0xE8)
        code.append(0x7F)
        code.append(0x01)
    ...
    def gen_rstack(self, operands: List[ASTNode]) -> bytes:
        ...
        # Classic ZIL/ZAP semantics: RSTACK returns from the current
        # routine with the value on top of the stack -> RET_POPPED
        # (0OP:0x08, 0xB8), exactly as the German-Zork .zap listings use it.
        # The old code emitted a stack-depth pseudo-read that fell through:
        # suspended's RFATAL macro <PROG () <PUSH 2> <RSTACK>> never
        # returned M-FATAL, so DEAD-FCN's REPEAT walked past the live robot
        # and declared every robot dead (game over on the first robot loss).
        return bytes([0xB8])
    ...
    def gen_do(self, operands: List[ASTNode]) -> bytes:
        ...
        if isinstance(start_node, NumberNode):
            ...
            start_code = self._generate_nested_and_adjust(start_node, code)
        ...
        if counting_up:
            ...
            if isinstance(end_node, NumberNode):
                ...
                try:
                    ...
                    while jump_offset < 0 and ((1 << 16) + jump_offset) in (0xFFAA, 0xFFAC, 0xFFBB):
                        code.insert(jump_pos, 0xB4)  # NOP pad: keep offset off marker values
                        jump_pos += 1
                        jump_offset -= 1
                    if jump_offset < 0:
    ...
    def gen_map_directions(self, operands: List[ASTNode]) -> bytes:
        ...
        while jump_offset < 0 and ((1 << 16) + jump_offset) in (0xFFAA, 0xFFAC, 0xFFBB):
            code.append(0xB4)  # NOP pad: keep offset off marker values
            current_pos += 1
            jump_offset -= 1
        if jump_offset < 0:
            jump_offset_unsigned = (1 << 16) + jump_offset
    ...
    def gen_equal(self, operands: List[ASTNode]) -> bytes:
        ...
        for op in operands:
            if isinstance(op, list):
                ...
                flattened.append(op)
        # <> as a comparand is the constant FALSE (0), not an expression --
        # generate_form emits no code for it, so the FormNode arms below would
        # compare against stale stack garbage (je sp,sp).
        operands = [NumberNode(0) if self._is_empty_false_form(op) else op
                    for op in flattened]
        ...
        if len(operands) < 2:
            ...
            return bytes([0xE8, 0x7F, 0x00])  # PUSH #0
        ...
        if len(operands) == 2:
            ...
            if needs_large1 or needs_large2:
                ...
                code.append(0xC8)  # branch on true, short, offset 8
        ...
        if op1_type == 1 and op1_val == 0 and len(all_comparands) > 3:
            # var 0x10 is the game's FIRST GLOBAL (zork3: HERE) --
            # use a dedicated scratch global instead.
        ...
        for idx, je_code in enumerate(je_instructions):
            ...
            offset = remaining_je_size + 3 + 3 + 2
    ...
    def _compare_materialize_tail(self, val_if_true: int, val_if_false: int) -> bytes:
        ...
        """
        tail = bytearray()
        tail.append(0xC8)                       # branch on true, short form, offset 8
        # FALSE block (compare false -> fall through): PUSH #val_if_false
        tail.extend([0xE8, 0x7F, val_if_false & 0xFF])
        # JUMP +5 over the TRUE block
        tail.extend([0x8C, 0x00, 0x05])
        # TRUE block (compare true -> branched here): PUSH #val_if_true
        tail.extend([0xE8, 0x7F, val_if_true & 0xFF])
        return bytes(tail)
    ...
    def gen_zero_test(self, operands: List[ASTNode]) -> bytes:
        ...
        if len(operands) == 1 and isinstance(operands[0], (FormNode, CondNode)):
            # Evaluate the nested form; the old path returned "stack" without
            # emitting its code, so jz popped leaked garbage (starcross
            # SYNTAX-CHECK's <0? <SET PREP <GET ...>>>).
            code = bytearray()
            if isinstance(operands[0], FormNode):
                code.extend(self.generate_form(operands[0]))
            else:
                code.extend(self.generate_cond(operands[0], value_context=True))
            code.append(0xA0)  # JZ stack
            code.append(0x00)
            code.extend(self._compare_materialize_tail(1, 0))
            return bytes(code)
        if len(operands) != 1:
    ...
    def gen_one(self, operands: List[ASTNode]) -> bytes:
        ...
        """
        if operands and isinstance(operands[0], (FormNode, CondNode)):
            code = bytearray()
            if isinstance(operands[0], FormNode):
                code.extend(self.generate_form(operands[0]))
            else:
                code.extend(self.generate_cond(operands[0], value_context=True))
            code.append(0x41)  # JE stack, 1
            code.append(0x00)
            code.append(0x01)
            code.extend(self._compare_materialize_tail(1, 0))
            return bytes(code)
    ...
    def _set_literal_number(self, op):
        """Return the literal int if op is <SET/SETG var <number>>, else None.
        ZILCH dialect: in predicate position a SET of a literal FIX is
        unconditionally TRUE (MDL: any FIX, even 0, is true; only FALSE is
        false) -- starcross PARSER's <AND <OR ...> <SET VAL 0>>."""
        if (isinstance(op, FormNode) and isinstance(op.operator, AtomNode)
                and op.operator.value.upper() in ('SET', 'SETG')
                and len(op.operands) == 2
                and isinstance(op.operands[1], NumberNode)):
            return op.operands[1].value
        return None

    def _emit_set_literal_true_tail(self, code, set_form, lit):
        """Emit the SET (store+push lit); normalize the pushed value to truthy
        (MDL's "truthy 0" has no Z-machine representation: push 1 instead)."""
        code.extend(self.generate_form(set_form))
        if lit == 0:
            if '_OR_SCRATCH_' not in self.globals:
                self.globals['_OR_SCRATCH_'] = self.next_global
                self.next_global += 1
            scratch = self.globals['_OR_SCRATCH_']
            code.append(0xE9); code.append(0x7F); code.append(scratch & 0xFF)  # PULL scratch
            code.append(0x14); code.append(0x00); code.append(0x01); code.append(0x00)  # ADD 0,1 -> sp
    ...
    def gen_and(self, operands: List[ASTNode]) -> bytes:
        ...
        if _lit is not None:
            ...
            if len(operands) == 1:
                ...
                return bytes(code)
            # <AND rest... set-lit>: evaluate <AND rest...>; false -> 0; else SET, true.
            code.extend(self.gen_and(operands[:-1]))
            code.append(0xA0); code.append(0x00)  # JZ sp
            _jz = len(code)
            ...
            _fail = len(code)
            code.append(0x14); code.append(0x00); code.append(0x00); code.append(0x00)  # push 0
            _end = len(code)
            _off = _fail - (_jz + 2) + 2
    ...
    def gen_queue(self, operands: List[ASTNode]) -> bytes:
        ...
        if isinstance(operands[0], AtomNode):
            ...
            if global_name not in self.globals:
                self._alloc_hard_or_raise(global_name)
                # Store placeholder for table address (self._table_marker(table_idx))
                self.global_values[global_name] = self._table_marker(table_idx); self.__dict__.setdefault('_tbl_marker_globals', set()).add(global_name)
    ...
    def get_tables_with_placeholders(self) -> List[Tuple[int, int]]:
        ...
        Tables like _VERBS contain self._table_marker(table_idx) placeholders that need to
